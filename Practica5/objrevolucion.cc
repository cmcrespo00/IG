...
void ObjRevolucion::crearMalla(std::vector<Tupla3f> perfil_original, int num_instancias) {
  ...
  v.push_back(v[0]);
  calcularCoordTextura(perfil_original, num_instancias);
}

void ObjRevolucion::calcularCoordTextura(std::vector<Tupla3f> perfil_original, int num_instancias){
  int N = num_instancias;
  int M = perfil_original.size();

  float modulo, divisor;

  std::vector<float> s, t, d;
  s.clear();
  s.resize(N);
  ...
  d[0] = 0;
  for (int j = 1; j < M; j++) {
    modulo = sqrt(pow((v[j][0] - v[j-1][0]) , 2) + pow((v[j][1] - v[j-1][1]) , 2) + pow((v[j][2] - v[j-1][2]) , 2));
    d[j] = d[j-1] + modulo;
  }
